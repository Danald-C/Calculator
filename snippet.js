			...
			/* console.log(".......................");
				...
				let entry = entries[n[0]];
						if(entry.entryBody.length == 0 || n[1] > entry.entryBody.length-1){
							tbStructure += "<td colspan=5>Entry Name: </td>";
						}else{
				...
				let entry = entries[n[0]];
						if(entry.entryBody.length == 0 || n[1] > entry.entryBody.length-1){
							...
							let computation = entry.entryBody[n[1]];
				// tbStructure += "<span>Entry "+n+" => "+computation+"</span>";
							let [operatorType, topOperator, number, operands, operand_type, answer] = processEntry(computation);
			...
			structure += "<div class='coll-apply-action-details'>";
				structure += "<textarea placeholder='Description (describe the item)'></textarea>";
				structure += "<inpput type='text' placeholder='Name (name the item)' />";
				structure += "<inpput type='date' />";
			structure += "</div>";
...
structure += "<li id='" + i + "' class='collection'>";
	  ...
	  // for (var j = 0; j < thisCollection[1].length; j++) {
		...
		structure += "<div class='coll-action-type'>";
		  ...
		  let thisTOperator = thisEntry.entryHead.topOperator;
		  if(entry_mode_data[0][1][2].length > 0){
			for(let k=0; k<entry_mode_data[0][1][2].length; k++){
				if(entry_mode_data[0][1][2][k][0] == i && entry_mode_data[0][1][2][k][1] == j){
		...
		// for (var k = 0; k < thisEntry[1].length; k++) {
		  ...
		  let [operatorType, topOperator, number, operands, mode, operand_type, answer] = processEntry(computation);
		  ...
		  structure += "<span class='figure'>" + filter_currency(number) + "</span>"; // Last total computation
		//   structure += "<span class='figure'>" + number + "</span>"; // Last total computation
	...
	let entry_mode_data = [[1, 1], []], newEntry = {entryHead: {name: "", topOperator: "0"}, entryBody: []}, newCollection = ["", [createNewDataSet(newEntry)]];
	$("div#random-entry a#add-new-coll").click(function () {
	//   $("div#random-entry ul#collection-set").css("display", "block");
	  $("div#random-entry div#collection-set").css("display", "block");
	  ...
	  $("div#random-entry div#collection-tab a").click(function () {
		...
		$("div#random-entry div#collection-set table.collection").removeClass("selected").css("display", "none");
		// $("div#random-entry ul#collection-set li.collection:nth-child("+$(this).attr("id")+")").addClass("selected").css("display", "block");
		$("div#random-entry div#collection-set table.collection:nth-child("+$(this).attr("id")+")").addClass("selected").css("display", "block");
  ...
  function random_entry(entry_mode_data, newEntry) {
	...
	//   structure += "<li id='" + i + "' class='collection'>";
	  structure += "<table id='" + i + "' class='collection'>";
		structure += "<thead class='coll-head'>";
			structure += "<tr>";
				structure += "<td>";
					structure += "<input type=='text' class='txt-inputs' style='width: 450px;' placeholder='";
					structure += thisCollection[0] == "" ? "Name this Collection.. (Optional)" : "Collection Name: " + thisCollection[0] + ". (Change here..)";
			...
			structure += "</tr>";
		structure += "</thead>";
		structure += "<tbody class='coll-body'>";
			thisCollection[1].map((entry, j) => {
			let thisEntry = entry;
	
			structure += "<tr class='coll-body-entry-"+j+"'>";
				structure += "<td id='name'>";
					structure += "<span class='figure'>" + (j + 1) + ".</span>";
					structure += "<input type='text' class='txt-inputs' placeholder='";
					structure += thisEntry.entryHead.name == "" ? "Name your Entry.. (Optional)" : /* : "Entry Name: " + */ thisEntry.entryHead.name + ". (Change here..)";
					structure += "' />";
				structure += "</td>";
				structure += "<td id='links'>";
					structure += "<a href='#' class='ordinary'>Save Name</a>";
					structure += "<a href='#' class='ordinary'>Reset</a>";
					structure += "<a href='#' class='remove'>Remove Entry</a>";
				structure += "</td>";
		...
		structure += "</tbody>";
	  structure += "</table>";
	//   structure += "</li>"; copecart.com / viralfindr.com
	}
	// structure += "<br style='clear: both;' />";
  
	// var elem_sel_init = "div#random-entry ul#collection-set", elem_sel_list = " li.collection.selected ul.coll-body li.coll-body-body";
	var elem_sel_init = "div#random-entry div#collection-set", elem_sel_list = " table.collection.selected tbody.coll-body tr:nth-child("+entry_mode_data[0][1]+")";
	$(elem_sel_init).html(structure);
	// $(elem_sel_init + " li.collection").removeClass("selected").css("display", "none");
	$(elem_sel_init + " table.collection").removeClass("selected").css("display", "none");
	// $(elem_sel_init + " li.collection:nth-child("+entry_mode_data[0][0]+")").addClass("selected").css("display", "block");
	$(elem_sel_init + " table.collection:nth-child("+entry_mode_data[0][0]+")").addClass("selected").css("display", "block");

	$(elem_sel_init + " table.collection").each(function () {
	...
	var sel_coll = $(elem_sel_init + " table.collection.selected"), i = sel_coll.attr("id");
	// sel_coll.find("ul.coll-body li.coll-body-body").each(function () {
	sel_coll.find("section.coll-body tr.coll-body-body").each(function () {
	...
	//   $(this).closest("li.coll-body-body").find("div.coll-apply-action div.coll-apply-action-head span.operator").text(operator);
	  $(this).closest("tr.coll-body-body").find("div.coll-apply-action div.coll-apply-action-head span.operator").text(operator);
	//   $(this).closest("li.coll-body-body").find("div.coll-apply-action div.coll-apply-action-head span.operand").html(sel_html);
	  $(this).closest("tr.coll-body-body").find("div.coll-apply-action div.coll-apply-action-head span.operand").html(sel_html);
	...
	// Set Names on Keyup
	// $(elem_sel_init + " li.collection.selected div.coll-head input, "+elem_sel_init + elem_sel_list + " div.coll-body-name input").keyup(function (event) {
	$(elem_sel_init + " table.collection.selected thead.coll-head tr td input, "+elem_sel_init + elem_sel_list + ":nth-child(1) td#name input").keyup(function (event) {
	  if (event.key == "Enter") {
		var label = $(this).val();
		if(label != ""){
		  let parentElement = $(this).closest('div').attr('class');
		  if(parentElement == "coll-head"){ // The Collection
			setCollectionName($(this), label);
		  }else{ // The Entry
			// var parent = $(this).closest("li.coll-body-body"), sel_coll = $(this).closest("li.collection"), i = sel_coll.attr("id"), j = parent.attr("id");
			var parent = $(this).closest("tr.coll-body-body"), sel_coll = $(this).closest("table.collection"), i = sel_coll.attr("id"), j = parent.attr("id");
	...
	// $(elem_sel_init + " li.collection.selected div.coll-head a").click(function () {
	$(elem_sel_init + " table.collection.selected thead.coll-head tr td a").click(function () {
	  ...
	  if($(this).index() == 1) { // Save Name
		// var label = $(this).closest("div.coll-head").find("input").val();
		var label = $(this).closest("thead.coll-head").find("tr td input").val();
	  ...
	  if($(this).index() == 2) { // Remove Collection
		...
		//   $("div#random-entry ul#collection-set").css("display", "none");
		  $("div#random-entry div#collection-set").css("display", "none");
		}
		random_entry(entry_mode_data, newEntry);
	  ...
	  if($(this).index() == 3) { // Add New Entry
		...
		var i = $(this).closest("table.collection").attr("id");
		// var j = entry_mode_data[1][i][1].length, parent = $(this).closest("li.coll-body-body:nth-child("+(j+1)+")");
		var j = entry_mode_data[1][i][1].length, parent = $(this).closest("tr.coll-body-body:nth-child("+(j+1)+")");
	...
	// Entry Links
	$(elem_sel_init + elem_sel_list + ":nth-child(1) div.coll-body-name-links a").click(function () {
	//   var parent = $(this).closest("li.coll-body-body"), entry_name = parent.find("div.coll-body-name input").val(), sel_coll = $(this).closest("li.collection"), i = sel_coll.attr("id"), j = parent.attr("id");
	  var parent = $(this).closest("tr.coll-body-body"), entry_name = parent.find("div.coll-body-name input").val(), sel_coll = $(this).closest("table.collection"), i = sel_coll.attr("id"), j = parent.attr("id");
	  ...
	  if ($(this).index() == 2) { // Remove Entry
		if (entry_mode_data[1][i][1].length > 1) {
		  ...
		  if(entry_mode_data[1][i][1].length > 0){
			...
			// $(this).closest("li.coll-body-body:nth-child("+(j)+")").addClass("active");
			$(this).closest("tr.coll-body-body:nth-child("+(j)+")").addClass("active");
	...
	$(elem_sel_init + elem_sel_list + " tr.coll-body-entry-2");
	...
	$(elem_sel_init + elem_sel_list + " div.coll-apply-action div.coll-apply-action-head span.perform a").click(function () {
	  ...
	  let [i, j] = getIndexes($(this));
	//   let thisEntry = entry_mode_data[1][i][1][j], topOperator = thisEntry.entryBody.length == 0 ? "0" : $(this).closest("li.coll-body-body").find("div.coll-action-type label.selected input[type=radio]").val();
	  let thisEntry = entry_mode_data[1][i][1][j], topOperator = thisEntry.entryBody.length == 0 ? "0" : $(this).closest("tr.coll-body-body").find("div.coll-action-type label.selected input[type=radio]").val();
	...
	//   var i = $(this).closest("li.collection").attr("id"), j = $(this).closest("li.coll-body-body").attr("id"), k = $(this).closest("li").attr("id");
	  var i = $(this).closest("table.collection").attr("id"), j = $(this).closest("tr.coll-body-body").attr("id"), k = $(this).closest("li").attr("id");
	...
	//   makeEntryActive($(this).closest("li.coll-body-body"), [0, j])
	  makeEntryActive($(this).closest("tr.coll-body-body"), [0, j])
	  random_entry(entry_mode_data, newEntry);
	...
	//   let [i, j] = [$(this).closest("li.collection").attr("id"), $(this).closest("li.coll-body-body").attr("id")];
	  let [i, j] = [$(this).closest("table.collection").attr("id"), $(this).closest("tr.coll-body-body").attr("id")];
	//   makeEntryActive($(this).closest("li.coll-body-body"), [i, j]);
	  makeEntryActive($(this).closest("tr.coll-body-body"), [i, j]);
  
	  if (event.key == "Enter") {
		var currency = digit_input_filter($(this).closest('span.operand').find('input'));
		// let topOperator = entry_mode_data[1][i][1][j].entryBody.length == 0 ? "0" : $(this).closest("li.coll-body-body").find("div.coll-action-type label.selected input[type=radio]").val();
		let topOperator = entry_mode_data[1][i][1][j].entryBody.length == 0 ? "0" : $(this).closest("tr.coll-body-body").find("div.coll-action-type label.selected input[type=radio]").val();
		let thisEntry = entry_mode_data[1][i][1][j];
	...
	//   var sel_coll = elem.closest("li.collection"), i = sel_coll.attr("id"), j = sel_coll.find("ul.coll-body li.coll-body-body.active").attr("id")
	  var sel_coll = elem.closest("table.collection"), i = sel_coll.attr("id"), j = sel_coll.find("section.coll-body tr.coll-body-body.active").attr("id")
	//   if(elem.closest("li.coll-body-body").attr("id") != j){
	  if(elem.closest("tr.coll-body-body").attr("id") != j){
		// j = elem.closest("li.coll-body-body").attr("id");
		j = elem.closest("tr.coll-body-body").attr("id");
		// makeEntryActive(elem.find("ul.coll-body li.coll-body-body"), [i, j]);
		makeEntryActive(elem.find("section.coll-body tr.coll-body-body"), [i, j]);
	  }
	  return [i, j]
	}
	...
	function makeEntryActive(parent, indexes=[0, 0]){
	  ...
	  if (!parent.is(".active") || entry_mode_data[0][1] !== j) {
		// $(elem_sel_init + " li.collection.selected ul.coll-body li.coll-body-body").removeClass("active");
		$(elem_sel_init + " table.collection.selected section.coll-body tr.coll-body-body").removeClass("active");
...
div#random-entry a:hover, div#random-entry div#collection-tab a.selected{
	/* background-color: #b3c6ff;
	transition: all 0.5s ease-out; */
}
div#random-entry div#collection-tab{
	margin-top: 20px;
	}
div#random-entry div#collection-tab a.selected{
	background-color: #b3c6ff;
	color: #fff;
	/* transition: .5s; */
	}
/* div#random-entry ul#collection-set{ */
div#random-entry div#collection-set{
	padding: 10px;
	margin: 0;
	border: 1px solid #668cff;
	}
/* div#random-entry ul#collection-set, div#random-entry ul#collection-set li.collection{ */
div#random-entry div#collection-set, div#random-entry ul#collection-set li.collection{
	display: none;
	}
/* div#random-entry ul#collection-set li.collection div.coll-head{ */
div#random-entry div#collection-set table.collection thead.coll-head tr td{
	background-color: rgba(179, 198, 255, .2);
	margin-bottom: 20px;
	padding: 10px;
	border: 1px solid #ccc;
	/* background-color: red; */
}
/* div#random-entry ul#collection-set ul.coll-body div.coll-action-type{ */
div#random-entry div#collection-set section.coll-body div.coll-action-type{
	background-color: #fff;
	margin: 10px 0;
	border: 1px solid #ccc;
}
/* div#random-entry ul#collection-set ul.coll-body div.coll-action-type label{ */
div#random-entry div#collection-set section.coll-body div.coll-action-type label{
	display: inline-block;
	padding: 10px 15px;
	cursor: pointer;
}
/* div#random-entry ul#collection-set ul.coll-body div.coll-action-type label.selected{ */
div#random-entry div#collection-set section.coll-body div.coll-action-type label.selected{
	background-color: #ffe8e8;
	transition: all 0.5s ease-out;
}
/* div#random-entry ul#collection-set ul.coll-body div.coll-apply-action div{ */
div#random-entry div#collection-set section.coll-body div.coll-apply-action div{
	display: inline-block;
	padding: 5px;
	}
/* div#random-entry ul#collection-set ul.coll-body{ */
div#random-entry div#collection-set section.coll-body{
	display: flex;
    flex-direction: row;
    flex-wrap: wrap;
	width: 100%;
...
/* div#random-entry ul#collection-set ul.coll-body li.coll-body-body{ */
div#random-entry div#collection-set section.coll-body tr.coll-body-body{
...
/* div#random-entry ul#collection-set ul.coll-body li.coll-body-body.active{ */
div#random-entry div#collection-set section.coll-body tr.coll-body-body.active{
	background-color: rgba(179, 198, 255, .2);
}
/* div#random-entry ul#collection-set ul.coll-body li.coll-body-body div.coll-body-name{ */
div#random-entry div#collection-set section.coll-body tr.coll-body-body div.coll-body-name{
	background-color: #fff;
}
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action div.coll-apply-action-head{ */
div#random-entry div#collection-set section.coll-body li div.coll-apply-action div.coll-apply-action-head{
	background-color: #fff;
	/* margin: 0; */
	padding: 0;
	border: 1px solid #ccc;
	}
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action div.coll-apply-action-head span, */
div#random-entry div#collection-set section.coll-body tr div.coll-apply-action div.coll-apply-action-head span,
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action ul.coll-apply-action-body li span/* , */
div#random-entry div#collection-set section.coll-body tr div.coll-apply-action ul.coll-apply-action-body td span{
	display: inline-block;
	padding: 10px;
	border: 1px solid #eee;
	}
/* div#random-entry ul#collection-set ul.coll-body span.figure{ */
div#random-entry div#collection-set section.coll-body span.figure{
	background-color: #fff;
	}
/* div#random-entry ul#collection-set ul.coll-body div.coll-apply-action div.coll-apply-action-head span.entry-total{ */
div#random-entry div#collection-set section.coll-body div.coll-apply-action div.coll-apply-action-head span.entry-total{
	font-size: 1.2em;
	font-weight: bold;
}
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action ul.coll-apply-action-body{ */
div#random-entry div#collection-set section.coll-body li div.coll-apply-action ul.coll-apply-action-body{
	/* background-color: red; */
	/* margin: 0; */
	padding: 0;
}
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action ul.coll-apply-action-body li{ */
div#random-entry div#collection-set section.coll-body tr div.coll-apply-action ul.coll-apply-action-body td{
	list-style: none;
}
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action ul.coll-apply-action-body li span{ */
div#random-entry div#collection-set section.coll-body tr div.coll-apply-action ul.coll-apply-action-body td span{
...
/* div#random-entry ul#collection-set ul.coll-body li div.coll-apply-action ul.coll-apply-action-body li span.s-n{ */
div#random-entry div#collection-set section.coll-body tr div.coll-apply-action ul.coll-apply-action-body td span.s-n{
	background-color: #ffe8e8;
}
/* div#random-entry ul#collection-set li.collection.selected ul.coll-body li.coll-body-body div.coll-apply-action div.coll-apply-action-head span.operand{ */
div#random-entry div#collection-set table.collection.selected section.coll-body tr.coll-body-body div.coll-apply-action div.coll-apply-action-head span.operand{
